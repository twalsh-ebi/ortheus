...
def remodelTreeRemovingRoot(root, node):
    ...
    def fn3(bT):
        if hash[bT] == root:
            s = '(' + ortheus.old.bioio.printBinaryTree(fn2(hash[bT], bT), bT, True)[:-1] + ')'
        else:
            s = '(' + ortheus.old.bioio.printBinaryTree(fn2(hash[bT], bT), bT, True)[:-1] + ',' + fn3(hash[bT]) + ')'
        return s + ":" + str(bT.distance)
    s = fn3(l[0]) + ';'
    t = ortheus.old.bioio.newickTreeParser(s)
    return t
...
def moveRoot(root, branch):
    ...
    def fn(tree, seq):
        ...
        if tree.internal:
            ...
            return ortheus.old.bioio.printBinaryTree(tree, True)[:-1]
    s = fn(root, None)
    return ortheus.old.bioio.newickTreeParser(s)
